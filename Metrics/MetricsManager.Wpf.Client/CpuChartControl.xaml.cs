        ...
        private void UpdateOnСlick(object sender, RoutedEventArgs e)
        {
            Task.Run(() => {
                while (true)
                {
                    ...
                    try
                    {
                        CpuMetricsWithAgentResponse cpuMetricsResponse =
                               _metricsManagerClient.GetCpuMetricsFromAgentAsync(1, fromTime.ToString("dd\\.hh\\:mm\\:ss"), toTime.ToString("dd\\.hh\\:mm\\:ss")).Result;
                        CpuMetricDto[] metrics = cpuMetricsResponse.Metrics.ToArray();
