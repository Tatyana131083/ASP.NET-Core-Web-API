        ...
        private void UpdateOnСlick(object sender, RoutedEventArgs e)
        {
            Task.Run(() => {
                while (true)
                {
                    ...
                    try
                    {
                        HddMetricsWithAgentResponse hddMetricsResponse =
                               _metricsManagerClient.GetHddMetricsFromAgentAsync(1, fromTime.ToString("dd\\.hh\\:mm\\:ss"), toTime.ToString("dd\\.hh\\:mm\\:ss")).Result;
                        HddMetricDto[] metrics = hddMetricsResponse.Metrics.ToArray();
