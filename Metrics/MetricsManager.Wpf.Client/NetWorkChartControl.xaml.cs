        ...
        private void UpdateOnСlick(object sender, RoutedEventArgs e)
        {
            Task.Run(() => {
                while (true)
                {
                    ...
                    try
                    {
                        NetworkMetricsWithAgentResponse networkMetricsResponse =
                               _metricsManagerClient.GetNetworkMetricsFromAgentAsync(1, fromTime.ToString("dd\\.hh\\:mm\\:ss"), toTime.ToString("dd\\.hh\\:mm\\:ss")).Result;
                        NetworkMetricDto[] metrics = networkMetricsResponse.Metrics.ToArray();
